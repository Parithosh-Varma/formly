...
- **Tables:** `forms` + `form_responses` `schema.sql:6` + `idx_form_responses_form(form_id)` + `CHECK` constraints post-harden.
...
- **Keys:** `sb_publishable_` vs `sb_secret_` naming confusion `server.js:10` — no startup assert `if(key.startsWith('sb_secret')) throw`.
- **Realtime:** `ws` transport `server.js:15` unused — `ws@8.21.3` adds CVE surface.

## Telegram
- `notifyTelegram` `server.js:359` fires on every `201` response, `signal timeout 8s`, concurrency `5`, `trunc(formTitle 60)` `q.label 80` `val 300`, `disable_web_page_preview true`.
- Token in `fetch https://api.telegram.org/bot${token}/sendMessage` URL — if client logs URL, leaks to `console.error` `server.js:359` (only `err.message` logged, safe).
...
- `ALLOWED_IPS` empty → `Set{}` → only loopback passes `server.js:30` — starves admin on remote.
